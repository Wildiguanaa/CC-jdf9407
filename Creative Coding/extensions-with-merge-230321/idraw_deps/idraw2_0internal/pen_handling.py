...
class PenHandler:
    ...
    def pen_raise(self, options, params, plot_status):
        ...
        if not options.preview:
            # drawcore_motion.sendPenUp(plot_status.port, v_time, params.servo_pin, False)
            drawcore_motion.sendPenUp(plot_status.port, options.pen_pos_up,options.pen_rate_raise,options.speed_penup)
        ...
        #         drawcore_motion.PBOutValue( plot_status.port, 3, 0, False) # I/O Pin B3 output: low
        self.status.pen_up = True
        # if not self.status.ebblv_set:
        #     drawcore_motion.setEBBLV(plot_status.port, options.pen_pos_up + 1, False)
    ...
    def pen_lower(self, options, params, plot_status):
        ...
        if not options.preview:
            # drawcore_motion.sendPenDown(plot_status.port, v_time, params.servo_pin, False)
            drawcore_motion.sendPenDown(plot_status.port, options.pen_pos_down,options.pen_rate_lower,options.speed_pendown)
            # if (v_time > 50) and (options.mode not in ["manual", "align", "toggle", "cycle"]):
            #     time.sleep(float(v_time - 30) / 1000.0) # pause before issuing next command
            # if params.use_b3_out:
            #     drawcore_motion.PBOutValue( plot_status.port, 3, 1, False) # I/O Pin B3 output: high
    ...
    def toggle(self, options, params, plot_status):
        ...
        if self.status.pen_up:
            speed = options.pen_rate_lower
        else:
            speed = options.pen_rate_raise
        drawcore_motion.TogglePen(plot_status.port,options.pen_pos_up,options.pen_pos_down,speed)
    ...
    def cycle(self, options, params, plot_status):
        ...
        drawcore_serial.command(plot_status.port, 'G4 P0.5\r')
    ...
    def servo_setup_wrapper(self, options, params, status):
        ...
        value = drawcore_motion.queryEBBLV(status.port, False)
        if value is None:
            return
        if int(value) != options.pen_pos_up + 1:
            # See "Methods" above for what's going on here.
            drawcore_motion.setEBBLV(status.port, 0, False)
            ...
            self.status.ebblv_set = True
            if drawcore_motion.QueryPenUp(status.port, False):
                self.status.pen_up = True
                self.status.virtual_pen_up = True
            else:
                self.status.pen_up = False
    ...
    def servo_setup(self, options, params, status):
        ...
        # drawcore_motion.setPenUpPos(status.port, int_temp, False)
        # int_temp = int(round(params.servo_min + servo_slope * self.heights.pen_pos_down))
        # drawcore_motion.setPenDownPos(status.port, int_temp, False)
        ...
        # drawcore_motion.setPenUpRate(status.port, int_temp, False)

        # int_temp = int(round(servo_rate_scale * options.pen_rate_lower))
        # drawcore_motion.setPenDownRate(status.port, int_temp, False)
        # drawcore_motion.servo_timeout(status.port, params.servo_timeout, None, False)
