...
class iDrawWrapperClass( inkex.Effect ):
    ...
    def __init__( self, default_logging = True, params = None ):
        if params is None:
            ...
            params = import_module("idraw2_0internal.idraw_conf") # Some settings can be changed here.
        self.params = params
    ...
    def effect( self ):
        ...
        if self.options.port_config == 3: # Use all available iDraw units.
            process_list = []
            EBBList = []
            EBBList = drawcore_serial.listDRAWCOREports()
            
            if EBBList:
                primary_port = None
                if self.options.port is not None:
                    primary_port = drawcore_serial.find_named(self.options.port)
                ...
                if len(EBBList) == 1:
                    logger.info("Found a single iDraw via USB.")
                    ...
                    for index, foundPort in enumerate(EBBList):
                        ...
                        if use_multiprocessing:  
                            process = multiprocessing.Process(target=self.plot_to_idraw, args=(foundPort[0],False))
                        else: # Use multithreading:
                            tname = "thread-" + str(index)
                            process = threading.Thread(group=None, target=self.plot_to_idraw, name=tname, args=(foundPort[0],False))
...
#             return # Skip secondary units, without opening class or serial connection
        ...
        selected_options = {item: self.options.__dict__[item] for item in ['mode',
            ...
            'layer', 'copies', 'page_delay', 'preview', 'rendering', 'model',
            'setup_type', 'resume_type', 'auto_rotate', 'resolution', 'reordering',
            'random_start', 'webhook', 'webhook_url', 'digest','plot_file',]}
        ...
        if primary:
            # Collect output from idraw.py 
            self.document = ad.document
            self.outdoc =  ad.get_output()
        else:
            if ad.error_out:
                if port is not None:
                    logger.error('Error on iDraw at port "' + port + '":' + ad.error_out)
                else:
                    logger.error('Error on secondary iDraw: ' + ad.error_out)
