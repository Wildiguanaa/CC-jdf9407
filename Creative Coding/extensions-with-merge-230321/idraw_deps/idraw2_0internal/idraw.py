...
drawcore_serial = from_dependency_import('drawcore_plotink.drawcore_serial')  # https://github.com/evil-mad/plotink
...
class iDraw(inkex.Effect):
    ...
    def update_options(self):
        ...
        # Physical travel bounds, based on iDraw2.0 model:
        ...
        self.speed_pendown = self.params.speed_pendown 
        self.speed_penup = self.params.speed_penup
        ...
        self.options.pen_rate_raise = \
            plot_utils.constrainLimits(self.options.pen_rate_raise, 1, 10000)
        self.options.pen_rate_lower = \
            plot_utils.constrainLimits(self.options.pen_rate_lower, 1, 10000)
        self.options.speed_pendown = plot_utils.constrainLimits(self.options.speed_pendown, 1, 10000)
        self.options.speed_penup = plot_utils.constrainLimits(self.options.speed_penup, 1, 12000)
    ...
    def effect(self):
        ...
        if self.options.mode == "manual":
            ...
            if self.options.manual_cmd == "list_names":
                self.name_list = drawcore_serial.list_named_ebbs() # Variable available for python API
                if not self.name_list:
                    self.user_message_fun(gettext.gettext("No named iDraw units located.\n"))
                else:
                    self.user_message_fun(gettext.gettext("List of attached iDraw units:"))
        ...
        if self.options.mode == "sysinfo":
            ports = drawcore_serial.listDRAWCOREports()
            if ports is None:
                logger.error("No available idraw units found on USB.")
                logger.error("Please check your connection(s) and try again.")
            else:
                msg = "There are {} iDarw devices connected to the computer:".format(len(ports))
                logger.error(msg)
                    ...
                    if self.plot_status.copies_to_plot != 0 and not self.plot_status.stopped:
                        ...
                        if self.options.preview:
                            ...
                            if self.plot_status.stopped:
                                self.plot_status.copies_to_plot = 0
                self.plot_status.progress.close()
        elif self.options.mode in ('align', 'toggle', 'cycle', 'raise_pen', 'lower_pen'):
            self.setup_command()
        ...
        if self.plot_status.port is not None:
            ...
            # drawcore_motion.sendDisableMotors(self.plot_status.port)#Disable Motors
    ...
    def setup_command(self):
        ...
        if self.options.mode == "align":
            ...
            # if self.pen_up:
            #     speed = self.options.pen_rate_lower
            # else:
            #     speed = self.options.pen_rate_raise
            # drawcore_motion.TogglePen(self.serial_port, self.options.pen_pos_up, self.options.pen_pos_down,speed)
    ...
    def manual_command(self):
        ...
        if self.options.manual_cmd == "fw_version":
            ...
            self.fw_version_string = drawcore_serial.queryVersion(self.plot_status.port)
        ...
        if (self.options.manual_cmd).startswith("write_name"):
            ...
            if drawcore_serial.min_version(self.plot_status.port, "2.5.5"):
                ...
                logger.error("This function requires a newer firmware version. See: idraw.com/fw")
            return

        # Next: Commands that require both power and serial connectivity:
        self.query_ebb_voltage()
        # Query if button pressed, to clear the result:
        drawcore_motion.QueryPRGButton(self.plot_status.port, False)
        if self.options.manual_cmd == "raise_pen":
            ...
            drawcore_motion.doMove(self.plot_status.port,self.y_bounds_max*25.4-2,0,5000)
            drawcore_serial.command(self.plot_status.port,'$SLP\r')
            ...
            if self.options.manual_cmd == "walk_home":
                return
                # if drawcore_serial.min_version(self.plot_status.port, "2.6.2"):
                ...
                #         n_delta_y *= 2
                # else:
                #     logger.error("This function requires newer firmware. Update at: idraw.com/fw")
                #     return
            ...
            self.plot_seg_with_v(-f_y, -f_x, 0, 0, ignore_limits=True)
    ...
    def plot_document(self):
        ...
        if not self.options.preview:
            ...
            if self.plot_status.port is None:
                return
            self.query_ebb_voltage()
            _unused = drawcore_motion.QueryPRGButton(self.plot_status.port, False) # Initialize button
    ...
    def plot_seg_with_v(self, x_dest, y_dest, v_i, v_f, ignore_limits=False):
        ...
        seg_logger.debug('speed_limit: {0}'.format(speed_limit))
        seg_logger.debug('vi_inch_per_s: {0}'.format(vi_inch_per_s))
            ...
            if move_steps1 != 0 or move_steps2 != 0:
                ...
                if not self.plot_status.resume.resume_mode and self.plot_status.stopped == 0:
                    ...
                    if self.options.preview:
                        ...
                        drawcore_motion.doXYMove(self.plot_status.port, move_steps2, move_steps1,\
                            move_time, False)
    ...
    def pause_res_check(self):
        ...
        if self.options.preview:
            ...
            str_button = drawcore_motion.QueryPRGButton(self.plot_status.port, False)
        ...
        if self.plot_status.stopped == 0 and self.plot_status.port is not None:
            try:
                ...
                self.user_message_fun(\
                    f'\nError: USB connection to iDraw2.0 lost. [Node {self.node_count}]\n')
    ...
    def enable_motors(self):
        ...
        if self.options.resolution == 1:  # High-resolution ("Super") mode
            ...
            self.speed_pendown = local_speed_pendown 
            self.speed_penup = self.options.speed_penup 
            if self.options.const_speed:
                self.speed_pendown = self.speed_pendown
        ...
        #     drawcore_motion.PBOutConfig(self.plot_status.port, 3, 0, False) # I/O Pin B3 -> output, low
