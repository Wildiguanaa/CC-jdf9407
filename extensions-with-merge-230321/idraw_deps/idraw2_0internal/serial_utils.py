...
def connect(options, plot_status, message_fun, logger):
    ...
    if not options.port: # Try to connect to first available iDraw.
        ...
        #the_port = drawcore_serial.find_named_ebb(options.port)
        plot_status.port = drawcore_serial.testPort(options.port)
    ...
    if plot_status.port is None:
        if port_name:
            message_fun('Failed to connect to iDraw ' + str(port_name))
        else:
            message_fun("Failed to connect to iDraw.")
        return False
    if port_name:
...
def query_voltage(options, params, plot_status, warnings):
    ...
    #     voltage_ok = drawcore_motion.queryVoltage(plot_status.port, False)
    #     if not voltage_ok:
