...
def QueryPenUp(port_name, verbose=True):
    ...
    if port_name is not None:
        pen_status = drawcore_serial.query(port_name, '$QP\r')
        if pen_status[0] == '0':
            return False
        return True
...
def sendPenDown(port_name, z_pos,speed,speed_pendown):
    if port_name is not None:
        str_output = 'G1G90 Z{0}F{1}\r'.format(z_pos, speed)
        drawcore_serial.command(port_name, str_output)
        drawcore_serial.command( port_name, 'G1 F{0}\r'.format(speed_pendown))


def sendPenUp(port_name, z_pos,speed,speed_penup):
    if port_name is not None:
        str_output = 'G1G90 Z{0}F{1}\r'.format(z_pos, speed)
        drawcore_serial.command(port_name, str_output)
        drawcore_serial.command( port_name, 'G1 F{0}\r'.format(speed_penup))
...
def TogglePen(port_name, pen_pos_up, pen_pos_down, speed):
    ...
    if port_name is not None:
        ...
        str_output = '$TP{0:0.1f},{1:0.1f}\r'.format(pen_pos_up, pen_pos_down)
        drawcore_serial.command(port_name, str_output)
