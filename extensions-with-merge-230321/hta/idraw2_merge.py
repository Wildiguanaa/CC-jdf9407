...
# ad_conf = import_module("axidraw_conf")
...
class iDrawMergeClass( inkex.Effect ):
    ...
    def __init__( self, default_logging = True, params = None, ad_params = None):
        ...
        self.OptionParser.add_option( "--submode", action="store", type="string",
            dest="submode", default="none", help="Secondary GUI tab" )

        self.version_string = "AxiDraw Merge v 5.2.1, 2022-10-01"
    ...
    def effect( self ):
        ...
        if self.options.mode == "options" and self.options.submode=="sysinfo":
            ...
            if ports is None:
                inkex.errormsg("No available idraw units found on USB.")
        ...
        ad.read_plotdata( self.svg )
        self.svg_row_old = ad.svg_row_old    # Access params from plotdata
        ...
        if self.options.mode == "auto_plot":
            ...
            if continue_plotting:
                ...
                for warning_message in ad.warnings.return_text_list():
                    inkex.errormsg(warning_message)
            ...
            elif ad.svg_layer_old == -1 :  # There appears to be a paused "all layers" plot
        ...
        if self.serial_port is not None:
            drawcore_motion.doTimedPause(self.serial_port, 10, False) # Pause for commands to finish.
            drawcore_serial.closePort(self.serial_port)
    ...
    def merge_and_plot( self, ha_ref, ad_ref ):
        ...
        selected_options = {item: self.options.__dict__[item] for item in ['pen_pos_up',
            ...
            'report_time', 'resolution', 'preview', 'rendering', 'model', 'reordering',
            'random_start', 'webhook', 'webhook_url',]}
        ad_ref.options.__dict__.update(selected_options)
        ad_ref.options.copies = 1
        #ad_ref.params = ad_conf   bug
    ...
    def pause_check (self):
        ...
        if self.options.preview:
            ...
            str_button = drawcore_motion.QueryPRGButton(self.serial_port, False) #Query button
        try:
    ...
    def modify_plotdata( self ):
        ...
        # Note: Whether to use xpath('plotdata') or xpath('//svg:plotdata', namespaces=inkex.NSS)
        #     is not necessarily obvious. Using just 'plotdata' apparently works in Inkscape.
        if nodes:
            data_node = nodes[0]
        if data_node is not None:
            try:
                ...
                data_node.set('randseed', str(self.svg_rand_seed)) # Random seed for this row
    ...
    def print_time_report( self, ad_ref ):
        ...
        ad_ref.plot_status.stats.report(self.options, inkex.errormsg, elapsed_time)
